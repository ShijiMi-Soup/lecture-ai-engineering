...
def init_db():
    ...
    try:
        ...
        st.toast(f"データベースの初期化に失敗しました: {e}", icon="❌")
...
def save_to_db(question, answer, feedback, correct_answer, is_correct, response_time):
    ...
    try:
        ...
        st.toast(f"データベースへの保存中にエラーが発生しました: {e}", icon="❌")
    finally:
        if conn:
            conn.close()
...
def get_chat_history():
    ...
    try:
        ...
        st.toast(f"履歴の取得中にエラーが発生しました: {e}", icon="❌")
...
def get_db_count():
    ...
    try:
        ...
        st.toast(f"レコード数の取得中にエラーが発生しました: {e}", icon="❌")
        return 0
    finally:
        if conn:
            conn.close()
...
def clear_db():
    ...
    try:
        ...
        conn.commit()
        st.toast("データベースが正常にクリアされました。", icon="✅")
        st.session_state.confirm_clear = False  # 確認状態をリセット
        return True  # 削除成功
    except sqlite3.Error as e:
        st.toast(f"データベースのクリア中にエラーが発生しました: {e}", icon="❌")
